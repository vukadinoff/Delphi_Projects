...

{
 ...
 Copyright(C) 2004-2010 Sasa Zeman. All Rights Reserved.
}
...
- Full MIME suppport
- UU and XX encoding/decoding functions
...
-------------------------
Version 1.4.1, 2010-12-12
-------------------------
  - Added Delphi 2009+ support (for Unicode versions of Delphi)
  - Added support for decoding buggy formed MIME with White space instead CRLF using 
    compiler directive WHITE_SPACE_SUPPORT since it is RFCs incompatible
    Thanks to Nils Bodeker

-------------------------
Version 1.4.0, 2009-09-25
-------------------------
  - UU encode/decode functions
  - XX encode/decode functions
...
// Allow white space support
//{$DEFINE WHITE_SPACE_SUPPORT}
...
function SZEncodeBaseXFile(const FileName: String; sOUT: TStream; const Codes: AnsiString; BITS: integer; FullQuantum : integer; MIMELine: integer): integer;
...
// UU Encode/decode
///////////////////////////////
function SZEncodeBaseUU(pIN, pOUT: PByte; Size: integer): integer; overload;
function SZEncodeBaseUU(sIN, sOUT: TStream; Size: integer=-1): integer; overload;
function SZEncodeBaseUU(const S: AnsiString): AnsiString; overload;
function SZDecodeBaseUU(pIN, pOUT: PByte; Size: integer): integer; overload;
function SZDecodeBaseUU(sIN, sOUT: TStream): integer; overload;
function SZDecodeBaseUU(const S: AnsiString): AnsiString; overload;

///////////////////////////////
// XX Encode/decode
///////////////////////////////
function SZEncodeBaseXX(pIN, pOUT: PByte; Size: integer): integer; overload;
function SZEncodeBaseXX(sIN, sOUT: TStream; Size: integer=-1): integer; overload;
function SZEncodeBaseXX(const S: AnsiString): AnsiString; overload;
function SZDecodeBaseXX(pIN, pOUT: PByte; Size: integer): integer; overload;
function SZDecodeBaseXX(sIN, sOUT: TStream): integer; overload;
function SZDecodeBaseXX(const S: AnsiString): AnsiString; overload;
...
///////////////////////////////////////////
  ...
  SZCodes64    : AnsiString = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/';
  SZCodes64URL : AnsiString = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-_';
  SZBITS64 = 6;
...
///////////////////////////////////////////
  ...
  SZCodes32 : AnsiString = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ234567';
  SZBITS32 = 5;
...
///////////////////////////////////////////
  ...
  SZCodes16 : AnsiString = '0123456789ABCDEF';
  SZBITS16 = 4;
...
begin
  ...
  if I8 > 0 then
  begin
    pOUT^ := Byte(Codes[ (B8 shl (BITS-I8)) + 1]);
    inc(pOUT);
    Result := 1;
  end else
    Result:=0;
...
{$IFDEF WHITE_SPACE_SUPPORT}

    else if pIN^ =  9 then inc(pIN)
    else if pIN^ = 32 then inc(pIN)
...
begin
  ...
  repeat
    if TotalIn > BUFF then
      Res := sIn.Read(pBuffIn^, BUFF)
    else
      Res := sIn.Read(pBuffIn^, TotalIn);
  ...
  until (TotalIn<=0) or (Res <> LongInt(BUFF)) ;
...
begin
  ...
  begin
    ...
    begin
      ...
      // Replaced code potantially incompatible for D2009+
...
const
  SZCodesUU : AnsiString = ' !"#$%&''()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\]^_';
  SZBITSUU = 6;

  // Resulted number of chars must be integral multiple of
  // 24 input bits div 6 output group bits
  // SZFullQuantumUU = 24 div 6;
  SZFullQuantumUU = 0;

function SZEncodeBaseUUMemoryUpdate(pIN: PByte; var pOUT: PByte; Size: integer; const Codes: AnsiString; BITS: integer; var vB8, VI8: integer; MIMELine: integer; Var MIMECountdown, MIMEBytesCount: integer): integer;
...
begin
  ...
  begin
    ...
    begin
      ...
      // Force changing space with chr(96)
      if (pOUT^ = 32)  then
        pOUT^ := 96;
...
function SZEncodeBaseUUMemory(pIN, pOUT: PByte; Size: integer; const Codes: AnsiString; BITS: integer; FullQuantum : integer): integer;
var
  B8,I8: integer;
...
begin
  ...
  ppOUT^ := Byte(Codes[Size + 1]); inc(ppOUT);

  TotalOut:=SZEncodeBaseUUMemoryUpdate(ppIN, ppOUT, Size, Codes, BITS, B8, I8, MIMECountdown, MIMECountdown, MIMEBytesCount);
...
function SZEncodeBaseUUString(const S: AnsiString; const Codes: AnsiString; BITS: integer; FullQuantum : integer): AnsiString;
var
  pIN, pOUT: PByte;
...
begin
  ...
  // Calculate relevant data
  GetRelevantData(TotalIn, TotalOut, BITS, FullQuantum, IM, 60);

  TotalOut:= TotalOut + (TotalIn div 45);

  if TotalIn mod 45 > 0 then
    inc(totalout);
  ...
  begin
    ...
    begin
      ...
      j:=SZEncodeBaseUUMemory(pIn,pOut, 45, Codes, BITS, FullQuantum);
      inc(LOut,j);
    ...
    begin
      ...
      j:=SZEncodeBaseUUMemory(pIn,pOut, i, Codes, BITS, FullQuantum);
      inc(LOut,j);
...
begin
  ...
  begin
    ...
    begin
      // M for UU, h for XX
      pOUT^:= Byte( SZCodesUU[45+1] ); inc(pOUT^);
      j:= SZEncodeBaseUUMemory(pIN, pOUT, 45, SZCodesUU, SZBITSUU, SZFullQuantumUU);
      inc(pOUT^, 45)
    end
    else
    begin
      pOUT^ := Byte( SZCodesUU[i+1] ); inc(pOUT^);
      j := SZEncodeBaseUUMemory(pIN,pOUT, i, SZCodesUU, SZBITSUU, SZFullQuantumUU);
...
function SZEncodeBaseUUStreamUpdate(sIN, sOUT: TStream; Size: integer; const Codes: AnsiString; BITS: integer; var vB8, vI8: integer; MIMELine: integer; Var MIMECountdown, MIMEByteCount: integer): integer;
var
  pBuffIn, pBuffOut: PByte;
  Res,BUFF : Integer;
  pIn,pOut: PByte;
...
begin
  ...
  repeat
    ...
    LOut:=SZEncodeBaseUUMemoryUpdate(pIn,pOut, Res, Codes, BITS, vB8, vI8, MIMELine, MIMECountdown, MIMEByteCount);
    sOut.Write(pBuffOut^,LOut);
...
function SZEncodeBaseUUStreamFinalyze(sOUT: TStream; const Codes: AnsiString; BITS: integer; var B8, I8: integer): integer;
var
  pOut: PByte;
  b: Byte;
begin
...
function SZEncodeBaseUUStream(sIN, sOUT: TStream; Size: integer; const Codes: AnsiString; BITS: integer; FullQuantum : integer; MIMELine : integer): integer;
var
  B8, I8: integer;
...
begin
  ...
  TotalOut:=SZEncodeBaseUUStreamUpdate(sIN, sOUT, TotalIn, Codes, BITS, B8, I8, MIMELine, MIMECountdown, MIMEBytesCount);
...
begin
  ...
  begin
    ...
    begin
      // M for UU, h for XX
      B:= Byte( SZCodesUU[45+1] );
      sOUT.Write(B,1);
      j:= SZEncodeBaseUUStream(sIN,sOUT, 45, SZCodesUU, SZBITSUU, SZFullQuantumUU,0);
    end
    else
    begin
      B:= Byte( SZCodesUU[i+1] );
      sOUT.Write(B,1);

      j:= SZEncodeBaseXStream(sIN,sOUT, i, SZCodesUU, SZBITSUU, SZFullQuantumUU,0);
    end;
...
{
function SZDecodeBaseUUMemoryUpdate(pIN, pOUT: PByte; Size: integer; const FastDecodeTable: TFastDecodeTable; BITS: integer; var B8, I8 : integer; var NewLine: Boolean): integer;
var
  len: Integer;
  TotalIN, TotalINLast, Count, clen: integer;
...
begin
  ...
  repeat
    ...
    if NewLine then
    begin
      // len contain number of expected output characters
      len := SZFastDecodeTable[pIN^-1];
    ...
    begin
      ...
      // if character is #96, conver it to space
      if B = 96 then B:=32;

      if SZFastDecodeTable[b] > 0 then
      begin

        B8 := B8 shl BITS;
        B8 := B8 or (SZFastDecodeTable[b]-1);
    ...
    if TotalINLast=TotalIN then break;
...
function SZDecodeBaseUUMemory(pIN, pOUT: PByte; Size: integer; const Codes: AnsiString; BITS: integer; var NewLine: Boolean): integer;
...
begin
  ...
  Result:=SZDecodeBaseUUMemoryUpdate( pIN, pOUT, Size, SZFastDecodeTable, BITS, B8, I8, NewLine);
end;

function SZDecodeBaseUUStream(sIN, sOUT: TStream;  const Codes: AnsiString; BITS: integer): integer;
var
  TotalIn  : integer;
  TotalOut : integer;
...
begin
  ...
  repeat
    ...
    Lout := SZDecodeBaseUUMemoryUpdate(pIN, pOUT, Res, SZFastDecodeTable, BITS, B8, I8, NewLine);
...
function SZDecodeBaseUUString(const S: AnsiString; const Codes: AnsiString; BITS: integer): AnsiString;
var
  TotalIn  : integer;
  TotalOut : integer;
...
begin
  ...
  TotalOut := SZDecodeBaseUUMemory(pIN, pOUT, TotalIn, Codes, BITS, NewLine);
...
function SZEncodeBaseUU(const S: AnsiString): AnsiString; overload;
begin
  Result := SZEncodeBaseUUString(S, SZCodesUU, SZBITSUU, SZFullQuantumUU)
end;
...
begin
  NewLine := true;
  Result := SZDecodeBaseUUMemory(pIN,pOUT, Size, SZCodesUU, SZBITSUU, NewLine);
end;


function SZDecodeBaseUU(sIN, sOUT: TStream): integer; overload;
begin
  Result := SZDecodeBaseUUStream(sIN,sOUT, SZCodesUU, SZBITSUU);
end;

function SZDecodeBaseUU(const S: AnsiString): AnsiString; overload;
begin
  Result := SZDecodeBaseUUString(S, SZCodesUU, SZBITSUU)
end;
}
//------------------------------------
// XX encoder / decoder
...
const
  ...
  SZCodesXX : AnsiString = '+-0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
  SZBITSXX  = 6;

  // Resulted number of chars must be integral multiple of
  // 24 input bits div 6 output group bits
  // SZFullQuantumXX = 24 div 6;
  SZFullQuantumXX = 0;
...
begin
  ...
  begin
    ...
    if i >= 45 then
    begin
      pOUT^ := Byte( SZCodesXX[45+1] ); inc(pOUT^);
      j := SZEncodeBaseUUMemory(pIN,pOUT, 45, SZCodesXX, SZBITSXX, SZFullQuantumXX);
      inc(pOUT^, 45)
    end
    else
    begin
      pOUT^ := Byte( SZCodesXX[i+1] ); inc(pOUT^);
      j := SZEncodeBaseUUMemory(pIN,pOUT, i, SZCodesXX, SZBITSXX, SZFullQuantumXX);
...
begin
  ...
  begin
    ...
    if i>=45 then
    begin
      B:= Byte(SZCodesXX[45+1]);
      sOUT.Write(B,1);
      j:= SZEncodeBaseUUStream(sIN,sOUT, 45, SZCodesXX, SZBITSXX, SZFullQuantumXX,0);
    end
    else
    begin
      B := Byte(SZCodesXX[i+1]);
      sOUT.Write(B, 1);
      j := SZEncodeBaseXStream(sIN,sOUT, i, SZCodesXX, SZBITSXX, SZFullQuantumXX,0);
    end;
...
function SZEncodeBaseXX(const S: AnsiString): AnsiString; overload;
begin
  Result := SZEncodeBaseUUString(S, SZCodesXX, SZBITSXX, SZFullQuantumXX);
end;
...
begin
  NewLine := true;
  Result := SZDecodeBaseUUMemory(pIN,pOUT, Size, SZCodesXX, SZBITSXX, NewLine);
end;

function SZDecodeBaseXX(sIN, sOUT: TStream): integer; overload;
begin
  Result := SZDecodeBaseUUStream(sIN,sOUT, SZCodesXX, SZBITSXX);
end;

function SZDecodeBaseXX(const S: AnsiString): AnsiString; overload;
begin
  Result := SZDecodeBaseUUString(S, SZCodesXX, SZBITSXX)
end;
}
initialization
