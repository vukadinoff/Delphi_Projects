...
function THttpClient.SendRequest(HttpRequest: THttpRequest; URL: String; Source, Response: TStream;
                                 CloseAfterRequest: Boolean=true): THttpRequestResult;
var FStrm : TFileStream;
begin
  FHttpClient.Request.ContentType     := 'text/xml';
  FHttpClient.Request.ContentVersion  := FContentVer;
  if (not CloseAfterRequest) then
    FHttpClient.Request.Connection := 'keep-alive'
   else
    FHttpClient.Request.Connection := '';
  ...
  try
    ...
    begin
      CloseConnection;
      Result     := hresSystem;
      FlastError := '[HTTP request] System error:' + E.Message;
    end;
  end;
end;
...
begin
  ...
  Responce := nil;
  try
    if (XML = '') then raise EAbort.Create('Empty XML document.)');
    ...
    on E: Exception do
     begin
       CloseConnection;
       FLastError := '[SOAP Request]' + E.Message + sLineBreak + '[HTTP URL] ' + URL;
       Result     := soapSystem;
     end;
  end;
