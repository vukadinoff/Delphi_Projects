...
type
  ...
  TIdHTTPChunkEvent = procedure(Thread: TIdPeerThread; const Size: Integer;
   ARequestInfo: TIdHTTPRequestInfo) of object;
  ...
  protected
    ...
    FOnChunkReceived: TIdHTTPChunkEvent;
    ...
    procedure DoCommandChunk(AThread: TIdPeerThread; const Size: Integer;
     ARequestInfo: TIdHTTPRequestInfo); virtual;
  ...
  published
    ...
    property OnChunkReceived: TIdHTTPChunkEvent read FOnChunkReceived write FOnChunkReceived;
  end;
...
implementation
uses
  IdCoderMIME, IdResourceStrings, IdURI, IdIOHandlerSocket, IdTCPConnection, Dialogs;
...
procedure TIdCustomHTTPServer.DoCommandChunk(AThread: TIdPeerThread; const Size: Integer;
   ARequestInfo: TIdHTTPRequestInfo);
begin
  if Assigned(FOnChunkReceived) then begin
    FOnChunkReceived(AThread, Size, ARequestInfo);
  end;
end;
...
var
  ...
  begin
    ...
    Result := StrToIntDef('$' + s, 0);      {do not localize}
  end;
         ...
         or (sCmd = 'HEAD')) and (Assigned(OnCommandGet) or FOkToProcessCommand) then begin    {Do not Localize}
          LRequestInfo := TIdHTTPRequestInfo.Create; try
            ...
            try
              ...
              // ÌÎÄÈÔÈÖÈÐÀÍÎ Ä.Ëóêàðåâ
              LRequestInfo.PostStream.Position := 0;
              if IndyPos('chunked', LowerCase(LRequestInfo.RawHeaders.Values['Transfer-Encoding'])) > 0 then
               begin // Chunked
                  Size := ChunkSize;
                  while Size > 0 do
                  begin
                    ReadStream(LRequestInfo.PostStream, Size);
                    ReadLn;
                    Size := ChunkSize;
                    DoCommandChunk(AThread, Size, LRequestInfo);
                  end;
                  ReadLn; // blank line
               end
              else
              if LRequestInfo.ContentLength > 0 then begin
                AThread.Connection.ReadStream(LRequestInfo.PostStream, LRequestInfo.ContentLength);
              end else begin
            ...
            LResponseInfo := TIdHTTPResponseInfo.Create(AThread.Connection); try
              ...
              // Session management
              GetSessionFromCookie(AThread, LRequestInfo, LResponseInfo, ContinueProcessing);
              // SG 05.07.99
